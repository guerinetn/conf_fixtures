...
=== Ecrivons le scnéario de notre histoire
...
[source,%numlines,yaml,highlight="1..23|2,5,7,11,17|18..21|18,19|22,23"]
----
demandeur:
  email: "john.doe@test"
  prenom: "John"
...
Certaines données sont importantes mais pour l'instruction pas pour l'application.
...
[source,%numlines,yaml,highlight="2..14|1|8..9|1..14"]
----
...
[NOTE.speaker]
====
On peut par contre ajouter une description histoire de savoir ce que fait le cas de test.
Même un périmètre d'intervention peut être limité aux données nécessaires.
...
[source, php]
----
$demandeConfig = $this->demandeFixturesService->checkAndPopulateConfig($file->getRealPath());
$description = isset($demandeConfig['description']) ? "\n  - {$demandeConfig['description']}" : '';
...
* Il ne reste plus qu'à parcourir les actions.
====
...
=== L'environnement d'intégration est tombé...

[NOTE.speaker]
====
Suite à un souci de persistence d'un container en environnement d'intégration, nous avons perdu toutes les données.
Attention aux `docker prune` abusifs.
Le mécanisme est inscrit dans l'application, rien n'empêche de l'utiliser sur d'autres environnements.
C'est tout naturellement que nous avons décidé de l'utiliser en environnement d'intégration.
====
